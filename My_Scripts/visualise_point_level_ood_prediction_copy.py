...
#     # def visualize_predictions(points, scores):
#     # """Visualize anomaly scores (jet colormap)."""
...
#     #points, _ = load_point_cloud("/media/ubuntu22/HDD22T/_Sourabh/stu_dataset/data/stu_dataset/validation/141/velodyne/000529.bin")
...
#     #scores = np.loadtxt("/media/ubuntu22/HDD22T/_Sourabh/stu_dataset/Mask4Former3D/saved/2025-07-14_151856/prediction/141/000529.txt", dtype=np.float32)
...
#     Seq = 125
#     Frame = 127
#     points, _ = load_point_cloud(f"/media/ubuntu22/HDD22T/_Sourabh/stu_dataset/data/stu_dataset/validation/{Seq}/velodyne/{Frame:06d}.bin")
...
#     scores = np.loadtxt(f"/media/ubuntu22/HDD22T/_Sourabh/PMF-master/experiments/PMF-SemanticKitti/log_SemanticKitti_PMFNet-resnet34_bs8-lr0.001_baseline_timestamp/Eval-SemanticKitti-PMFNet-best_IOU_model-noKNN-debug_timestamp/preds/uncertainty_maxlogit/{Seq}/{Frame:06d}.txt", dtype=np.float32)
#     # points = load_point_cloud("/media/ubuntu22/HDD22T/_Sourabh/PMF-master/data/semantic-kitti-fov/sequences/125/velodyne/000127.bin")
#     # scores = np.loadtxt("/media/ubuntu22/HDD22T/_Sourabh/PMF-master/experiments/PMF-SemanticKitti/log_SemanticKitti_PMFNet-resnet34_bs8-lr0.001_baseline_timestamp/Eval-SemanticKitti-PMFNet-best_IOU_model-noKNN-debug_timestamp/preds/uncertainty_maxlogit/125/ml_000127.txt")
...
def load_point_cloud(pcd_file):
    ...
    num_points = points.shape[0]
    print(f"Loaded {num_points} points from {pcd_file}")
    return points[:, :3]  # Return only XYZ, not tuple
...
class PointOODVisualizer:
    ...
    def _apply_distance_mask(self, points, labels):
        """Filter labels based on distance (original logic)."""
        # Ensure points is a proper numpy array
...
def visualize_single_frame(points, scores):
    ...
    if isinstance(points, tuple):
        # If points is a tuple (from old load_point_cloud), take first element
        points = points[0] if len(points) > 0 else points
    
    points = np.asarray(points)
    if points.ndim != 2 or points.shape[1] != 3:
        print(f"❌ Invalid points shape: {points.shape}. Expected [N, 3]")
        return
    
    print(f"📊 Visualizing {len(points)} points with {len(scores)} scores")
...
if __name__ == "__main__":
    Seq = 137
    Frame = 343
    base_path = "/media/ubuntu22/HDD22T/_Sourabh/PMF-master/experiments_semkitti_stu/PMF-SemanticKitti/log_SemanticKitti_PMFNet-resnet34_bs8-lr0.001_baseline_timestamp/Eval-SemanticKitti-PMFNet-best_IOU_model-noKNN-debug_timestamp/preds_all_methods/"
    method ="rba"

    # Load point cloud - FIXED: assign only the points, not the tuple
    points = load_point_cloud(f"/media/ubuntu22/HDD22T/_Sourabh/stu_dataset/Mask4Former3D/data/test/{Seq}/velodyne/{Frame:06d}.bin")
    # Load uncertainty scores
    #this is for semkitti
    # scores = np.loadtxt(f"/media/ubuntu22/HDD22T/_Sourabh/PMF-master/Results/preds_ml_msp_undistorted_image/uncertainty_maxlogit/{Seq}/ml_{Frame:06d}.txt")
    #this is for semkitti+stu
    # scores = np.loadtxt("/media/ubuntu22/HDD22T/_Sourabh/stu_dataset/Mask4Former3D/saved/2025-11-16_144825/prediction_query_based/125/000137.txt")
    # scores = np.loadtxt(f"/media/ubuntu22/HDD22T/_Sourabh/stu_dataset/Mask4Former3D/saved/2025-12-02_155230/prediction_query_based_msp/{Seq}/{Frame:06d}.txt") #with -1 multiplied
    scores = np.loadtxt(f"/media/ubuntu22/HDD22T/_Sourabh/stu_dataset/Mask4Former3D/saved/2025-12-02_155230/prediction_msp/{Seq}/{Frame:06d}.txt") #without -1 multiplication

    # Debug info
    print(f"Points type: {type(points)}, shape: {points.shape if hasattr(points, 'shape') else 'No shape'}")

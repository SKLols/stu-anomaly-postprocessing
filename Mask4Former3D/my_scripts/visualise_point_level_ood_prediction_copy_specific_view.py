...
class PointOODVisualizer:
    ...
    def visualize_predictions(self, points, scores):
        """Visualize anomaly scores (jet colormap) with custom camera view."""
        ...
        # Normalize scores and map to jet colors
        normalized_scores = (scores - scores.min()) / (scores.max() - scores.min() + 1e-8)
        jet = cm.get_cmap("jet")
        score_colors = jet(normalized_scores)[:, :3]
        ...
        vis.create_window(window_name="Anomaly Scores (Predictions)")
        vis.add_geometry(pcd)
        vis.add_geometry(coord_frame)

        # Set custom view: look from rotated direction
        theta = math.radians(30)  # 30° clockwise around Y axis
        front = [-math.cos(theta), 0, math.sin(theta)]  # From -X rotated clockwise
        up = [0, 0, 1]  # Z up
        lookat = [0, 0, 0]  # Look at origin
        zoom = 0.05

        # Get view control and apply
...
if __name__ == "__main__":
    points, _ = load_point_cloud("/media/ubuntu22/HDD22T/_Sourabh/stu_dataset/data/stu_dataset/validation/141/velodyne/000320.bin")
    scores = np.loadtxt("/media/ubuntu22/HDD22T/_Sourabh/stu_dataset/Mask4Former3D/saved/2025-07-14_151856/prediction/141/000320.txt", dtype=np.float32)
    #scores = np.loadtxt("/media/ubuntu22/HDD22T/_Sourabh/stu_dataset/batch_0_max_logit_final_maxlogit.txt", dtype=np.float32)


    #points = load_point_cloud("/media/ubuntu22/HDD22T/_Sourabh/stu_dataset/Mask4Former3D/data/test/201/velodyne/000000.bin")
    #scores = np.loadtxt("/media/ubuntu22/HDD22T/_Sourabh/stu_dataset/Mask4Former3D/saved/2025-07-14_220257/prediction/201/000000.txt")

...
class Mask4Former(nn.Module):
    ...
    def apply_react(self, sparse_tensor, trunc_percentile=0.95):
        """
        Apply ReAct to MinkowskiEngine SparseTensor features
        Only applied during evaluation to reduce overconfidence on anomalies
        """
        if not isinstance(sparse_tensor, me.SparseTensor) or not hasattr(sparse_tensor, 'F'):
            return sparse_tensor
        
        features = sparse_tensor.F
        # Calculate threshold for each feature channel
        thresholds = torch.quantile(features, trunc_percentile, dim=0)
        # Truncate activations above threshold
        truncated_features = torch.clamp(features, min=thresholds)
        
        # Return new sparse tensor with truncated features
    ...
    def forward(self, x, raw_coordinates=None, is_eval=False):
        ...
        #print("all_features:",all_features[-1].shape)
        # Apply ReAct to backbone features during evaluation
        # if is_eval:
        #     for i in range(len(all_features)):
        #         all_features[i] = self.apply_react(all_features[i], trunc_percentile=0.15)

        point_features = self.point_features_head(all_features[-1])

        # # Also apply ReAct to point features during evaluation
        # if is_eval:
        #     point_features = self.apply_react(point_features, trunc_percentile=0.15)

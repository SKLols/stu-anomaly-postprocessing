...
class PointOODMetricsCalculator:
    ...
    def compute_metrics(self):
        ...
        #plt.hist(predictions[targets == 0], bins=50, alpha=0.5, label='Inliers')
        plt.hist(predictions[targets == 1], bins=50, alpha=0.5, label='Anomalies')
        plt.legend(); plt.xlabel('Anomaly Score'); plt.ylabel('Count')
        plt.savefig('score_distribution.png')

        print("Labels:", np.unique(targets, return_counts=True))
        print("predictions_labels:", np.unique(predictions, return_counts=True))
